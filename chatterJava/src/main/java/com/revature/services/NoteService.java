...
public class NoteService {
	...
	public static void getNoteByName(String notename){
	 ...
	 * @return
	 */
	public static NoteDto insertNoteDto(String loc, UserDto ownerDto, NoteType type, String name) {
		// create a new NoteDaoImpl
		nd = new NoteDaoImpl();
		logger.info("NoteDaoImpl created");
		...
		User owner = ud.selectUserById(1);
		...
		Note toInsert = new Note(0, owner, type, loc, name);
	 ...
	 * @return
	 */
	public static NoteDto selectNoteDtoByName(String name) {
		// create a new NoteDaoImpl
		nd = new NoteDaoImpl();
		logger.info("NoteDaoImpl created");

		// get the record that with id as the primary key as a Note Object
		Note note = nd.selectNoteByName(name);
		logger.info("Note selected from database");
		logger.debug("Note: " + note);
